...
{  
  ...
  if (myVertex.isValid()) {
    ...
    int trkSize = thePrimaryV.tracksSize(); // test num of trk with and without muonLessPV (KYO)
    myCand.addUserInt("trkSize", trkSize); // test num of trk with and without muonLessPV (KYO)
...
//      iEvent.getByLabel("hiGenParticles", theGenParticles); // KYO embedded
